...
saveRDS(perm_rescue_aso, paste0("RData/perm_rescue_", aso0, ".rds"))
...
ggplot(df[c(1, 2, 4, 5), ], aes(x = category, y = count, fill = class, label = label)) + 
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 15)) + 
  theme(axis.text.x = element_text(angle = 90, hjust=1))
ggsave(paste0("pdf/barplot_XDPDEG_2Sets_and_", aso0, "_Rescued.jpg"), width = 5.7, height = 4.2)
...
ggplot(df[1:2, ], aes(x = category, y = count, fill = class, label = label)) + 
  ...
  scale_x_discrete(labels = function(x) str_wrap(x, width = 15)) + 
  theme(axis.text.x = element_text(angle = 90, hjust=1))
ggsave("pdf/barplot_XDPDEG_2Sets.jpg", width = 5.7, height = 4.2)
...
df_up = data.frame(FC = c(mean(exp(res_xv_perm_up$LFC.Genotype1XDP_vanilla)), 
                          ...
                          mean(exp(res_trt_perm_up[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_repl_up[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_stri_up[[paste0("LFC.", term_word)]]))),
...
df_dn = data.frame(FC = c(mean(exp(res_xv_perm_dn$LFC.Genotype1XDP_vanilla)), 
                          ...
                          mean(exp(res_trt_perm_dn[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_repl_dn[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_stri_dn[[paste0("LFC.", term_word)]]))),
                   category = c("Permissive XDP Gene Signatures", 
                                "Replicated XDP Gene Signatures", 
                                "Stringent XDP Gene Signatures", 
                                "Permissive XDP Gene Signatures", 
...
df_up = data.frame(FC = c(mean(exp(res_xv_perm_rescue_aso_up$LFC.Genotype1XDP_vanilla)), 
                          ...
                          mean(exp(res_trt_perm_rescue_aso_up[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_repl_rescue_aso_up[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_stri_rescue_aso_up[[paste0("LFC.", term_word)]]))),
...
df_dn = data.frame(FC = c(mean(exp(res_xv_perm_rescue_aso_dn$LFC.Genotype1XDP_vanilla)), 
                          ...
                          mean(exp(res_trt_perm_rescue_aso_dn[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_repl_rescue_aso_dn[[paste0("LFC.", term_word)]])), 
                          mean(exp(res_trt_stri_rescue_aso_dn[[paste0("LFC.", term_word)]]))),
                   category = c("Permissive XDP Gene Signatures Rescued", 
                                "Replicated XDP Gene Signatures Rescued", 
                                "Stringent XDP Gene Signatures Rescued", 
                                "Permissive XDP Gene Signatures Rescued", 
...
df = data.frame(FC = c(c(exp(res_xv_perm_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_up$LFC.Genotype1XDP_vanilla)),
                       ...
                       c(exp(res_xv_perm_rescue_aso_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_rescue_aso_up$LFC.Genotype1XDP_vanilla)),
                       c(exp(res_xc_repl_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_repl_rescue_aso_up$LFC.GenotypeXDP)),
                       c(exp(res_xc_stri_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_stri_rescue_aso_up$LFC.GenotypeXDP)),
                       
                       c(exp(res_trt_perm_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_perm_rescue_aso_up[[paste0("LFC.", term_word)]])),
                       c(exp(res_trt_repl_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_repl_rescue_aso_up[[paste0("LFC.", term_word)]])),
                       c(exp(res_trt_stri_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_stri_rescue_aso_up[[paste0("LFC.", term_word)]]))),
                ...
                category = c(rep("Permissive XDP Gene Signatures", length(c(exp(res_xv_perm_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_up$LFC.Genotype1XDP_vanilla)))),
                             ...
                             rep("Permissive XDP Gene Signatures", length(c(exp(res_xv_perm_rescue_aso_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_rescue_aso_up$LFC.Genotype1XDP_vanilla)))), 
                             rep("Replicated XDP Gene Signatures", length(c(exp(res_xc_repl_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_repl_rescue_aso_up$LFC.GenotypeXDP)))), 
                             rep("Stringent XDP Gene Signatures", length(c(exp(res_xc_stri_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_stri_rescue_aso_up$LFC.GenotypeXDP)))), 
                             
                             rep("Permissive XDP Gene Signatures", length(c(exp(res_trt_perm_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_perm_rescue_aso_up[[paste0("LFC.", term_word)]])))), 
                             rep("Replicated XDP Gene Signatures", length(c(exp(res_trt_repl_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_repl_rescue_aso_up[[paste0("LFC.", term_word)]])))), 
                             rep("Stringent XDP Gene Signatures", length(c(exp(res_trt_stri_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_stri_rescue_aso_up[[paste0("LFC.", term_word)]]))))),
                ...
                class = c(rep("Permissive all", length(c(exp(res_xv_perm_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_up$LFC.Genotype1XDP_vanilla)))),
                          ...
                          rep(paste0(aso0, "-rescued"), length(c(exp(res_xv_perm_rescue_aso_dn$LFC.Genotype1XDP_vanilla), exp(res_xv_perm_rescue_aso_up$LFC.Genotype1XDP_vanilla)))), 
                          rep(paste0(aso0, "-rescued"), length(c(exp(res_xc_repl_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_repl_rescue_aso_up$LFC.GenotypeXDP)))), 
                          rep(paste0(aso0, "-rescued"), length(c(exp(res_xc_stri_rescue_aso_dn$LFC.GenotypeXDP), exp(res_xc_stri_rescue_aso_up$LFC.GenotypeXDP)))), 
                          
                          rep(paste0(aso0, "-rescued in treated vs untreated XDP"), length(c(exp(res_trt_perm_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_perm_rescue_aso_up[[paste0("LFC.", term_word)]])))), 
                          rep(paste0(aso0, "-rescued in treated vs untreated XDP"), length(c(exp(res_trt_repl_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_repl_rescue_aso_up[[paste0("LFC.", term_word)]])))), 
                          rep(paste0(aso0, "-rescued in treated vs untreated XDP"), length(c(exp(res_trt_stri_rescue_aso_dn[[paste0("LFC.", term_word)]]), exp(res_trt_stri_rescue_aso_up[[paste0("LFC.", term_word)]]))))))

df$class = factor(df$class, levels = c("Permissive all", "Replicated all", "Stringent all", paste0(aso0, "-rescued"), paste0(aso0, "-rescued in treated vs untreated XDP")))
df = df[which(!grepl("Stringent", df$category)), ]

p = ggplot(df, aes(x = class, y = 100* (FC - 0))) + 
  geom_sina(aes(color = class), size = 0.1, position = position_dodge(1), maxwidth = 0.5, scale = "width") + 
...
p
ggsave(paste0("pdf/sinaplot_FC_XDPDEG_rescue_2Sets_against_", aso0, "_white.jpg"), width = 6.1, height = 4.24)

p + scale_color_manual(name = "class", labels = function(x) str_wrap(x, width = 15), values = c("#0066FF", "#EC8014", "darkgrey", "pink"))
ggsave(paste0("pdf/sinaplot_FC_XDPDEG_rescue_2Sets_against_", aso0, ".jpg"), width = 6.1, height = 4.24)
...
p = go_change_plot(perm_go_sel, deg_aso_p_go, "Permissive XDP Gene Signatures", paste0("Nominal ", aso0, " Gene Signatures"), "query_annotated", "pvalue", 0.05, ntop = 20, text_width = 30, text_size = 6, transform_val = F, show_hit = F)
