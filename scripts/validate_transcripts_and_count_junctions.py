...
def sj_fpkm(fa,root,sj,ps,phe,cap,cf,cut):
    ...
    for x in ps:
        ...
        while i<len(start)-1:
            ...
            for x in sjl:
	        keys=x.split("/")[-2]
                #keys0=x.split("STAR.")[1].split(".")[0][8:]
                #keys1=x.split("/")[-3]
                #keys=keys1+"_"+keys0
...
#        for y in sjl:
#            if sa == y.split("STAR.")[1].split(".")[0][8:] and batch==y.split("/")[-3].split("_")[1] and ("capture" in y):
#                sample=y[:-1]
#                if fam in dic:
#                    dic[fam].append(sample)
#                else:
    ...
    for x in w:          # x: trancript
        ...
        for fam in dic:
            ...
            for sample in dic[fam]:
                logf=sample.split("/")[-2]
		#logf0=sample.split("STAR.")[1].split(".")[0][8:]
                ...
                #if np.min(saj)>float(cut) and ("rnaseq" in logf):   # only transripts validated in RNASeq counts, not Capture
...
cmp("/data/talkowski/Samples/XDP/Brain_RNASeq/data/Trinity/merged/trinity_CAU_merged/Trinity_1line.fasta",
    "/data/talkowski/Samples/XDP/Brain_RNASeq/data/Trinity/merged/trinity_CAU_merged",
    "/data/talkowski/Samples/XDP/Brain_RNASeq/data/Trinity/merged/trinity_CAU_merged/CAU_SJ_list",
    "/data/talkowski/Samples/XDP/Brain_RNASeq/data/Trinity/merged/trinity_CAU_merged/Design_matrix_CAU.txt",
    "CAU",
    "/data/talkowski/Samples/XDP/Brain_RNASeq/data/Trinity/merged/trinity_CAU_merged/Caudate/all/pre_allSamples_SJ.fpkm.processed.matrix",
    "0.00001")
